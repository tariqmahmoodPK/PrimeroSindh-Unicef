...
class NotificationMailer < ApplicationMailer
  ...
  def transition_notify(transition_id)
    ...
    @child = Child.find_by(id: record.id) || (return log_not_found('Case', record.id))
    @sig = @child.data["is_this_a_significant_harm_case__b343242"]
    mail(
  ...
  def send_response_notification(child_id, user_email, role)
    @child_id = child_id
    @role = role
    mail(
      to: user_email, subject: ('Notification or case response')
    )
  end
