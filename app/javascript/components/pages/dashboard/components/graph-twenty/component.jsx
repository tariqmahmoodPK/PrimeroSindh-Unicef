...
import { fetchGraphTwenty } from "../../action-creators";
import { getGraphTwenty } from "../../selectors";
...
const Component = () => {
  ...
  const data = useMemoizedSelector(state => getGraphTwenty(state));
  const stats = data.getIn(["data", "stats"]) ? data.getIn(["data", "stats"]).toJS() : null;

  useEffect(() => {
    dispatch(fetchGraphTwenty());
  }, []);
...
Component.displayName = `GraphTwenty`;
