...
import { fetchGraphNineteen } from "../../action-creators";
import { getGraphNineteen } from "../../selectors";
...
const Component = () => {
  ...
  const data = useMemoizedSelector(state => getGraphNineteen(state));
  const stats = data.getIn(["data", "stats"]) ? data.getIn(["data", "stats"]).toJS() : null;

  useEffect(() => {
    dispatch(fetchGraphNineteen());
  }, []);
...
Component.displayName = `GraphNineteen`;
