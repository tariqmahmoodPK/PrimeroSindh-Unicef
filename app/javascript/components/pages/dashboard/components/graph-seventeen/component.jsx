...
import { fetchGraphSeventeen } from "../../action-creators";
import { getGraphSeventeen } from "../../selectors";
...
const Component = () => {
  ...
  const data = useMemoizedSelector(state => getGraphSeventeen(state));
  const stats = data.getIn(["data", "stats"]) ? data.getIn(["data", "stats"]).toJS() : null;

  useEffect(() => {
    dispatch(fetchGraphSeventeen());
  }, []);
...
Component.displayName = `GraphSeventeen`;
