...
import { fetchGraphEighteen } from "../../action-creators";
import { getGraphEighteen } from "../../selectors";
...
const Component = () => {
  ...
  const data = useMemoizedSelector(state => getGraphEighteen(state));
  const stats = data.getIn(["data", "stats"]) ? data.getIn(["data", "stats"]).toJS() : null;

  useEffect(() => {
    dispatch(fetchGraphEighteen());
  }, []);
...
Component.displayName = `GraphEighteen`;
