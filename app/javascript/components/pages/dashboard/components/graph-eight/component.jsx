...
import { fetchGraphEight } from "../../action-creators";
import { getGraphEight } from "../../selectors";
...
const Component = () => {
  ...
  const data = useMemoizedSelector(state => getGraphEight(state));
  const stats = data.getIn(["data", "stats"]) ? data.getIn(["data", "stats"]).toJS() : null;

  useEffect(() => {
    dispatch(fetchGraphEight());
  }, []);
...
Component.displayName = `GraphEight`;
