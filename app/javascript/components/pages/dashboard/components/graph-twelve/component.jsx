...
import { fetchGraphTwelve } from "../../action-creators";
import { getGraphTwelve } from "../../selectors";
...
const Component = () => {
  ...
  const data = useMemoizedSelector(state => getGraphTwelve(state));
  const stats = data.getIn(["data", "stats"]) ? data.getIn(["data", "stats"]).toJS() : null;

  useEffect(() => {
    dispatch(fetchGraphTwelve());
  }, []);
...
Component.displayName = `GraphTwelve`;
