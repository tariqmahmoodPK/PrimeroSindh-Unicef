...
class Transition < ApplicationRecord
  ...
  after_commit :update_case_workflow
  ...
  def in_progress?
    return status == Transition::STATUS_DONE if type == "Assign"
  ...
  def update_case_workflow
    return unless type.eql?("Referral")
    child = Child.find(record_id)

    return if child.referrals.many?

    child.update_case_worflow_to_referrals
  end
