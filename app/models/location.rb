...
class Location < ApplicationRecord
  ...
  scope :pluck_location_placename, -> { all.pluck(:location_code, :placename_i18n).map { |code, placename| { "#{code}": placename["en"] }} }
  scope :with_type_district, -> { where(type: "district").pluck('location_code') }
  scope :with_type_province, -> { where(type: "province").pluck('location_code') }
  scope :pluck_location, -> { pluck(:location_code) }
  scope :get_location_all_towns, -> { find_by_sql("SELECT id, location_code, type, admin_level, placename_i18n FROM locations WHERE type = 'tehsil'")}
  scope :get_location_towns, ->(location_code) { find_by_sql("SELECT id, location_code, type, admin_level, placename_i18n FROM locations WHERE type = 'tehsil' AND location_code LIKE '#{location_code}%'")}
  scope :pluck_location_districts, -> { find_by_sql("SELECT id, location_code, type, admin_level, placename_i18n FROM locations WHERE type = 'district'") }
