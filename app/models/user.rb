...
class User < ApplicationRecord
  ...
  scope :users_and_role_with_location, -> { includes(:role).where.not(location: nil) }
  ...
  validates :agency, presence: { message: 'errors.models.user.organization' }, if: :service_account
  ...
  def self.social_service_workforce_by_district(user)
    return { permission: false } unless user.role.unique_id == "role-cp-manager"

    users = { "labels" => [], "data" => [] }
    user_data = {}
    location_hash = Location.pluck_location_placename
    location_codes = User.users_and_role_with_location.group([:location]).size.keys

    User.users_and_role_with_location.find_each do |user|
      unless user_data["#{user.role.name}"]
        user_data["#{user.role.name}"] = {} unless user_data["#{user.role.name}"]
        location_codes.each do |code|
          user_data["#{user.role.name}"][code] = 0
        end
      end
      user_data["#{user.role.name}"]["#{user.location}"] += 1
    end
    ...
    users["labels"] = users["labels"].compact

    rgba_code = [0, 255, 128]
    user_data.each do |key, value|
      first_code = rgba_code.sample
      second_code = rgba_code.sample
      third_code = rgba_code.sample
      users["data"] << {
                          name: key,
                          data: value.values,
                          backgroundColor: "rgba(#{ first_code }, #{ second_code }, #{ third_code }, 0.5)",
                          borderColor: "rgba(#{ first_code }, #{ second_code }, #{ third_code })"
                        }
    end
    users
  end

  def self.staff_by_gender_stats(user)
    return { permission: false } unless user.role.name.eql?("CP Manager")

    users = User.by_agency(user.agency_id)
    stats = [users.male.size, users.female.size, users.transgender.size]
